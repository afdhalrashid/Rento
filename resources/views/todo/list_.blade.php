@foreach ($todoLists as $key => $todo)
    <div class="task high">
        ...
        <div class="time">
            ...
            <div class="date">Tarikh Perlu Siap:
                {{ $todo->dateline !== '0000-00-00 00:00:00' ? \Carbon\carbon::parse($todo->dateline)->format('d M Y') : '-' }}
            </div>
            {{-- <?php
            ...
            <div style="margin: 0 auto;">
                @if (array_key_exists($todo->id, $status))
                    ...
                    <span class="badge badge-pill badge-warning">
                        {{ $status[$todo->id]['pending'] ?? 0 }} Belum siap
                    </span>

                    <span class="badge badge-pill badge-success">
                        {{ $status[$todo->id]['completed'] ?? 0 }} Siap
                    </span>
                @endif
            </div>
            {{-- <div>{{$status[$todo->id]['completed'] ?? 0}} Completed</div> --}}
            <div style="font-size: 14px"><a href="/todos/{{ $todo->id }}?delete='title'" class="far fa-trash-alt"
                    data-taskId="{{ $todo->id }}"></a></div>
        </div>
    </div>
@endforeach
