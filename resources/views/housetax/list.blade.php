...
@section('content')
            ...
            <div class="page-title">
                <div class="row">
                    <div class="col-lg-3 col-md-3 order-md-1 order-last">
                        <h4>Cukai</h4>
                        <p class="text-subtitle text-muted">Senarai Cukai Rumah</p>
                    </div>
            ...
            <section class="section">
                <div class="row">
                    ...
                    <div class="col-sm-9 col-md-9 col-lg-9">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Senarai Cukai</h4>
                            </div>
                            <div class="card-body">
                                <table id="listhouse_table" class="table table-striped">
                                    ...
                                    @foreach ($house->tax as $tax)
                                        ...
                                        <tr>
                                            ...
                                            <td class="">
                                                {{ $tax->month }}</td>
                                            <td class="">{{ date('d M Y', strtotime($tax->payment_date)) }}</td>
                                                                        <td class="
                                            ...
                                            <td class="
                                                ...
                                                @if ($tax->image_path != '')
                                                    ...
                                                    {{-- <a class="btn-sm
                                                btn-success"
                                                        href="{{ route('downloadtax', $tax->id) }}">Download</a>
                                                    <a class="btn-sm btn-info" href="{{ route('viewtax', $tax->id) }}"
                                                        target="_blank">Open</a> --}}
                                                        <a class="btn-sm btn-success" data-toggle="tooltip"
                                                        data-placement="top" title="Muat turun"
                                                            href="{{ route('downloadtax', $tax->id) }}"><i
                                                            class="far fa-arrow-alt-circle-down"></i></a>
                                                        <a class="btn-sm btn-info" data-toggle="tooltip"
                                                        data-placement="top" title="Buka" href="{{ route('viewtax', $tax->id) }}"
                                            ...
                                            <td>
                                                ...
                                                <button type="button" class="btn btn-danger btn-sm"
                                                    onclick="loadDeleteModal('{{ $house->id }}','{{ $tax->id }}','{{ $tax->tax_type }}', '{{ $tax->year }}', '{{ $tax->month }}', '{{ $tax->value }}')"><i class="far fa-trash-alt"></i>
                                                </button>
    ...
    <form action="{{ route('housetax.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('POST')
                ...
                <div class="modal-body">
                    <div class="container">
                        ...
                        <div class="row">
                            <div class="col-sm-3 col-md-3 col-lg-3 px-0">
                                <div class="form-group">
                                    <label for="helpInputTop">Jenis Cukai</label>
                                    <small class="text-muted">cth.<i> Cukai Pintu</i></small>
                                    {{ old('tax_type') }}
                                    <select class="form-control form-select-sm shadow-sm"
                                        aria-label="Default select example" name="tax_type">
                                        <option value="" selected>- Sila Pilih -</option>
                                        @foreach ($global_housetax_types as $tax_type)
                                            @if (Request::old('tax_type') == $tax_type->type_id)
                                                <option value="{{ $tax_type->type_id }}" selected>
                                                    {{ $tax_type->type_name }}</option>
                                            @else
                                                <option value="{{ $tax_type->type_id }}">
                                                    {{ $tax_type->type_name }}</option>
                                            @endif
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-sm-3 col-md-3 col-lg-3">
                                <div class="form-group">
                                    ...
                                    {{ old('tax_tahun') }}
                                    <select class="form-control form-select-sm shadow-sm"
                                        aria-label="Default select example" name="tax_tahun">
                                        <option value="" selected>- Sila Pilih -</option>
                                        @php
                                            $years = get_list_year();
                                        @endphp
                                        @foreach ($years as $year)
                                            @if (Request::old('tax_tahun') == $year)
                            ...
                            <div class="col-sm-2 col-md-2 col-lg-2 px-0">
                                <div class="form-group">
                                    ...
                                    {{ old('tax_bulan') }}
                                    <select class="form-control form-select-sm shadow-sm"
                                        aria-label="Default select example" name="tax_bulan">
                                        <option value="" selected>- Sila Pilih -</option>
                                        @php
                                            $months = get_list_month();
                                        @endphp
                                        @foreach ($months as $month)
                                            @if (Request::old('tax_bulan') == $month)
                            ...
                            <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
                                <div class="form-group">
                                    ...
                                    <small class="text-muted">cth.<i> 28/03/2020</i></small>
                                    <div class="datepicker date input-group p-0 shadow-sm">
                                        <input name="tarikh_bayar_tax" type="text" placeholder="Pilih tarikh"
                                            class="form-control py-1 px-3" id="tarikh_bayar_tax">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group mb-3">
                                        ...
                                        <input type="number" step=".01" class="form-control shadow-sm py-5 px-5 text-center"
                                            id="tax_jumlah" name="tax_jumlah" value="{{ old('tax_jumlah') }}"
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-12 col-md-12 col-lg-12 col px-0">
                                <div class="form-group">
                                    ...
                                    <textarea type="text" class="form-control shadow-sm" id="tax_remark" name="tax_remark"
                        ...
                        <div class="row custom-file">
                            <input type="file" class="custom-file-input" id="tax_attachment" name="tax_attachment"
                                value="{{ old('tax_attachment') }}" multiple="">
                            <label class="custom-file-label" for="tax_attachment">Choose file</label>
                        </div>
                    </div>
                </div>
...
@section('js')
    <script>
        setInputFilter(document.getElementById("tax_jumlah"), function(value) {
            // return /^\d*$/.test(value);
        ...
        $('[name="tax_jumlah"]').on('keyup', function() {
            limitText(this, 8)
        });
        ...
        function loadDeleteModal(house_id, tax_id, jenis, tahun, bulan, jumlah) { //not complete yet
            $('#modal_delete_house2 .modal-title').text('Anda pasti untuk hapus cukai berikut: ');
            $('#modal_delete_house2 .modal-body #maklumat_owner').html('<p>Jenis Kos: ' + jenis + '</p>' + '<p>Tahun: ' +
                tahun + '</p>' + '<p>Bulan: ' + bulan + '</p>' + '<p>Jumlah : RM' + jumlah + '</p>');
            $('#modal_delete_house2 #modal-confirm_delete').attr('onclick', 'confirmDelete(' + house_id + ',' + tax_id +
                ')');
            $('#modal_delete_house2').modal('show');
        }

        function confirmDelete(houseid, tax_id) {
            console.log(tax_id);
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $("#modal_delete_house2 .msg").slideDown(500).delay(500).fadeOut(500, 'swing', closeModal);
